...
function init() {
   ...
   function startGame() {
      ...
      for (let i = 1; i <= 8; i++) {
         ...
         let blockNumber = scrambledOrder[i - 1];
         if (blockNumber) {
            if (currentBlock.firstElementChild) {
               currentBlock.id = "block" + blockNumber;
               currentBlock.firstElementChild.src = "images/block" + blockNumber + ".jpg";
               currentBlock.firstElementChild.alt = blockNumber;
            } else {
               console.error(`Block ${i} is missing an <img> element.`);
            }
         } else {
            console.error(`scrambledOrder[${i - 1}] is undefined.`);
         }
      }
